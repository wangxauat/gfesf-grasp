...
def parse_args():
    ...
    # Network
    parser.add_argument('--network', metavar='N', type=str,
                        nargs='+', help='Path to saved networks to evaluate')
    ...
    parser.add_argument('--dataset-path', type=str, default=r'E:\grasp_detect\dataset\cornell',
...
if __name__ == '__main__':
    ...
    for network in args.network:
        ...
        with torch.no_grad():
            for idx, (x, y, didx, rot, zoom) in enumerate(val_data):
                xc = x.to(device)
                ...
                if args.iou_eval:
                    s = evaluation.calculate_iou_match(q_img, ang_img, val_data.dataset.get_gtbb(didx, rot, zoom),
                ...
                if args.jacquard_output:
                    ...
                    with open(jo_fn, 'a') as f:
                        for g in grasps:
                            f.write(val_data.dataset.get_jname(didx) + '\n')
                            f.write(g.to_jacquard(scale=1024 / 300) + '\n')

                if args.vis:
                    save_results(
                        rgb_img=val_data.dataset.get_rgb(didx, rot, zoom, normalise=False),
                        depth_img=val_data.dataset.get_depth(didx, rot, zoom),
                        grasp_q_img=q_img,
        ...
        avg_time = (time.time() - start_time) / len(val_data)
